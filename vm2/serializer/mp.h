...
public:
	enum datatype {
		...
		INTEGER = msgpack::type::POSITIVE_INTEGER,
		DOUBLE = msgpack::type::DOUBLE,
		BLOB = msgpack::type::RAW,
		ARRAY = msgpack::type::ARRAY,
		MAP = msgpack::type::MAP,
		STRING = msgpack::type::RAW,
	};
...
#define CHECK_LENGTH(len)	if ((m_c + len) >= m_s) { 	\
	...
	ASSERT(false);										\
	return NBR_ESHORT; }
	inline int pushnil() { CHECK_LENGTH(1); push(NIL_VALUE); return m_c; }
...
#define CHECK(require)	\
	...
	public:
		...
		operator bool () const { ASSERT(CHECK(BOOLEAN)); return via.boolean; }
		operator int () const { ASSERT(CHECK(msgpack::type::POSITIVE_INTEGER)||
			CHECK(msgpack::type::NEGATIVE_INTEGER)); return (int)via.i64; }
		operator unsigned int () const { ASSERT(CHECK(msgpack::type::POSITIVE_INTEGER)|| 
			CHECK(msgpack::type::NEGATIVE_INTEGER)); return (unsigned int)via.u64; }
		operator long long () const { ASSERT(CHECK(msgpack::type::POSITIVE_INTEGER)||
			CHECK(msgpack::type::NEGATIVE_INTEGER)); return via.i64; }
		operator unsigned long long () const { ASSERT(CHECK(msgpack::type::POSITIVE_INTEGER)||
			CHECK(msgpack::type::NEGATIVE_INTEGER)); return via.u64; }
		operator double() const { CHECK(DOUBLE); return via.dec; }
		operator const char *() const { CHECK(BLOB); return via.raw.ptr; }
		operator const void *() const { CHECK(BLOB); return via.raw.ptr; }
	};
#undef CHECK_TYPE
protected:
	unpacker m_upk;
